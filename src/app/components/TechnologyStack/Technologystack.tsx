            ...
            <div className="max-w-[1320px] mx-auto">
                ...
                <div >
                    <ul className="flex flex-row flex-wrap gap-4 my-10">
                        {tabsContent.map((i) =>
                            <li key={i.tabName} className={`px-4 w-auto h-[50px] flex items-center font-medium text-base sm:text-lg leading-[21.83px] rounded-full cursor-pointer ${i.isActive ? "bg-primary text-white" : "bg-lightRed text-black"} `}
                ...
                <div className='py-6 '>
                    {tabsContent.map((tech) =>
                          <div key={tech.tabName} className={`flex flex-row flex-wrap gap-4 md:gap-6 ${tech.isActive ? "opacity-100 max-h-[800px] lg:max-h-[500px]" : "opacity-0 max-h-0"} overflow-hidden transition-all duration-500 ease-in-out`}>
                        {tech.childern.map((item) =>
                            <div key={item.techName} className={`bg-lightRed max-w-[120px] sm:max-w-[166px] w-full h-auto flex flex-col justify-center items-center py-4 sm:py-8 rounded-[24px] sm:rounded-[40px] `}>
